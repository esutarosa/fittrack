...
pub fn router(state: AppState) -> Router {
    Router::new()
        ...
        .route(
            ...
            patch(training::update_exercise).delete(training::delete_exercise),
        )
        .route("/api/progress/{exercise_id}", get(training::list_progress))
        .route("/api/workouts", get(training::list_workouts).post(training::create_workout))
        .route(
            "/api/workouts/{workout_id}",
            patch(training::update_workout).delete(training::delete_workout),
        )
        .route(
        ...
        .route(
            "/api/workouts/{workout_id}/sets/{workout_set_id}",
            patch(training::update_workout_set).delete(training::delete_workout_set),
        )
